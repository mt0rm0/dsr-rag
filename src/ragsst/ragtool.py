...
class RAGTool:
    ...
    def set_collection(self, collection_name: str, embedding_model: str | None = None) -> None:
        ...
        logger.info(
            f'Set collection: {self.collection_name}. '
    ...
    def make_collection(
        ...
        Args:
            ...
            consider_content:     Re-ingest files whose content has changed.
        """
        ...
        if skip_included_files:
            ...
            files_hashes = {m.get('file_hash') for m in existing_meta} if consider_content else set()
    ...
    def _filter_query_by_keyword(self, query_result: dict, keyword: str) -> dict:
        ...
        if not relevant_docs:
            return {}
        query_result['metadatas'][0] = [
            meta for meta, doc in zip(query_result['metadatas'][0], query_result['documents'][0])
            if kw in doc.lower()
        ]
        query_result['documents'][0] = relevant_docs
        return query_result
